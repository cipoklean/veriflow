 ...
 * navigate away. Per-tx progress shimmer + elapsed seconds + clickable
 * explorer chip. Confirmation/reversion toasts fire here, not in pages.
 */
export function TxDockProvider({ children }: { children: ReactNode }) {
  ...
  const track = useCallback((hash: string, label: string) => {
    setTxs(prev => (prev.some(t => t.hash === hash) ? prev : [...prev, { hash, label, status: 'pending' as const, startedAt: Date.now() }]));
  }, []);

  const confirm = useCallback((hash: string) => {
    setTxs(prev => {
      const tx = prev.find(t => t.hash === hash);
      if (tx && tx.status !== 'confirmed') {
        toast({ title: 'Transaction confirmed', description: tx.label, type: 'success', duration: 5000 });
      }
      return prev.map(t => (t.hash === hash ? { ...t, status: 'confirmed' as const, confirmedAt: Date.now() } : t));
    });
  }, [toast]);
  ...
  const revert = useCallback((hash: string) => {
    setTxs(prev => {
      ...
      return prev.map(t => (t.hash === hash ? { ...t, status: 'reverted' as const, confirmedAt: Date.now() } : t));
    });
  }, [toast]);
...
function TxChip({ tx, onRemove }: { tx: TxRecord; onRemove: (h: string) => void }) {
  ...
  const elapsed = Math.max(0, Math.round((Date.now() - tx.startedAt) / 1000));
